...
| ------- | -------- | -------- | ------------ |
| T030 | 检查更新升级为静默更新：启动自动检查发现新版本后自动后台下载（失败重试 3 次后本次会话放弃，手动检查不受限）；退出时自动应用已就绪更新包（替换脚本不重启，下次启动即新版），脚本增加防竞态（检测到应用已重新拉起则中止复制）；下载完成提示统一为“更新包已就绪，重启即更新”；关于页“立即更新”按钮保留（重启立即生效）。Coordinator 抽取 DownloadCoreAsync 供手动/自动复用，缓存路径与各段等待时长经 internal 构造函数可注入；下载前清理被中断残留的 staging 目录；退出钩子放 Program.Main（StartWithClassicDesktopLifetime 返回后线程池限时 10s 执行，避免 UI 调度器停转后的异步续延死锁）；BuildUpdateScript 重构为纯静态函数便于单测。验证：新增单测 12 个（coordinator 状态机 8 + 脚本文案/staging 清理 4），64/64 通过；另有 %TEMP% 下控制台 harness 驱动真实 AppUpdateService + 真实 PowerShell 的端到端三场景（退出静默复制、重启模式复制后拉起、同名进程守卫中止）全部通过 | DONE | 2026-09-17 |
