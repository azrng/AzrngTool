...
- 发现实现方案与设计文档、DTO 契约、需求基准或设计系统不一致时，应主动提醒并建议同步修订
- 维护文档时优先更新已有事实来源，不新增平行说明；确需新增文件时，应说明新增原因和归档位置
- 对不确定的业务事实、DTO 字段、配置项和运行方式，只能标记为待确认或引用已验证来源，禁止写成确定结论
...
- **回退规则**：后续阶段发现 DTO 缺口时，按根 `AGENTS.md` 的纠错与回退机制回到契约变更流程，不在实现层绕过契约。
...
- 文件目录：`doc/devlog/`
- 文件格式：Markdown
- 文件命名建议：`YYYY-MM-DD-HHmmss-中文任务简述.md`
...
- 归档操作步骤：
  1. 只筛选可归档的 `DONE` 任务，不处理仍需关注的任务。
  2. 将较早的 `DONE` 任务迁入对应月份的 `doc/task-archive/YYYY-MM.md`。
  3. `TASK.md` 仅保留当前任务、待确认任务、阻塞任务与最近少量已完成任务。
  4. 若新增了归档文件，补充到 `TASK.md` 的归档索引。
